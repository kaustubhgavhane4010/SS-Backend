...
console.log('🧠 SMART DATABASE INITIALIZATION - Protecting your data...');

async function smartInitDatabase() {
  try {
    ...
    if (fs.existsSync(dbPath)) {
      ...
      try {
        ...
        if (userCount?.count > 0 && orgCount?.count > 0) {
          console.log('🛡️ DATABASE HAS DATA - PRESERVING EVERYTHING!');
          console.log('💡 Your data is safe, no initialization needed');
    ...
    console.error('❌ Smart database initialization failed:', error.message);
    throw error;
  }
}
